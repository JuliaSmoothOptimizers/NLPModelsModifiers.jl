@testset "NoFixedModel NLP tests" begin
  @testset "API T=$T, M=$M" for T in [Float64, Float32], M in [NLPModelMeta] # , SimpleNLPMeta
    f(x) = (x[1] - 2)^2 + (0 - 1)^2
    ∇f(x) = T[2 * (x[1] - 2)]
    H(x) = 2 * ones(T, 1, 1)
    c(x) = T[x[1] - 2 * 0 + 1; -x[1]^2 / 4 - 0 + 1]
    J(x) = reshape(T[1, -0.5x[1]], 2, 1)
    H(x, y) = H(x) + y[2] * T[-0.5]

    nlp = NoFixedModel(SimpleNLPModel(T, M, fixed_var = true))
    n = nlp.meta.nvar
    m = nlp.meta.ncon
    @test nlp.meta.x0[1:n] == T[2]
  ...
  @testset "Show" begin
    nlp = NoFixedModel(SimpleNLPModel())
    io = IOBuffer()
    show(io, nlp)
    showed = String(take!(io))
    expected = """NoFixedModel{Float64, Vector{Float64}, SimpleNLPModel{Float64, Vector{Float64}, NLPModelMeta{Float64, Vector{Float64}}}, NLPModelMeta{Float64, Vector{Float64}}} - A NLPModel without fixed variables
  Problem name: Simple NLP Model (no fixed variables)
