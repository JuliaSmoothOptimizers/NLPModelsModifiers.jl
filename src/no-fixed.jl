...
@doc raw"""A model whose fixed variables are removed.

Fixed variables are variables whose lower and upper bounds are equal.
"""
...
function NoFixedModel(model::AbstractNLPModel{T, S}) where {T, S}
  ...
  hess_rows = Vector{Int}(undef, get_nnzh(model))
  hess_cols = Vector{Int}(undef, get_nnzh(model))
  ...
  try
    ...
    for i = 1:get_nnzh(model)
      if !(ifix_mask[old_rows[i]] || ifix_mask[old_cols[i]])
        count_nnzh += 1
        mask[count_nnzh] = i
        hess_rows[count_nnzh] = full_to_red[old_rows[i]]
        hess_cols[count_nnzh] = full_to_red[old_cols[i]]
      end
    end
  catch
    is_implemented_hessian = false
  end
  resize!(hess_rows, count_nnzh)
  resize!(hess_cols, count_nnzh)
  ...
  return NoFixedModel(
    ...
    similar(x_copy),
    similar(x_copy),
    is_implemented_hessian,
    hess_rows,
    hess_cols,
    mask,
    similar(x_copy, get_nnzh(model)),
    jac_lin_rows,
    jac_lin_cols,
    jac_nln_rows,
    jac_nln_cols,
    similar(x_copy, get_nnzj(model)),
    jac_lin_mask,
    jac_nln_mask,
  )
end
...
"""
    transform_x(nlp::NoFixedModel, x::AbstractVector)

Given a vector `x` of length `nlp.meta.nvar`, return a vector with fixed variables included.
"""
function transform_x(nlp::NoFixedModel{T, S}, x::S) where {T, S}
  ...
  j = 0
  @inbounds @simd for i = 1:get_nvar(nlp.model)
    if nlp.ifix_mask[i]
...
"""
    transform_v!(nlp::NoFixedModel, v::AbstractVector, v_long::AbstractVector)

Given a vector `v` of length `nlp.meta.nvar`, return a vector filled with zeros for fixed variables indices.
"""
...
"""
    get_from_temp!(nlp::NoFixedModel, temp::AbstractVector, g::AbstractVector)

Given a vector `temp` of length `nlp.model.meta.nvar`, return a vector `g` with fixed variables removed.
"""
...
NLPModels.obj(nlp::NoFixedModel, x::AbstractVector; kwargs...) =
  obj(nlp.model, transform_x(nlp, x); kwargs...)
for meth in (:cons!, :cons_lin!, :cons_nln!)
...
for meth in (:jtprod!, :jtprod_lin!, :jtprod_nln!)
  @eval function NLPModels.$meth(
    nlp::NoFixedModel,
    x::AbstractVector,
    v::AbstractVector,
...
function NLPModels.jac_lin_coord!(nlp::NoFixedModel, x::AbstractVector, vals::AbstractVector)
  NLPModels.jac_lin_coord!(
    nlp.model,
    transform_x(nlp, x),
    view(nlp.temp4, 1:(get_lin_nnzj(nlp.model))),
  )
...
function NLPModels.jac_nln_coord!(nlp::NoFixedModel, x::AbstractVector, vals::AbstractVector)
  NLPModels.jac_nln_coord!(
    nlp.model,
    transform_x(nlp, x),
    view(nlp.temp4, 1:(get_nln_nnzj(nlp.model))),
  )
...
function NLPModels.hess_coord!(
  ...
  if !nlp.is_implemented_hessian
    throw(
      ErrorException("Not implemented hess_structure! for original problem $(typeof(nlp.model))"),
    )
  end
  hess_coord!(nlp.model, transform_x(nlp, x), nlp.temp3; kwargs...)
  @inbounds @simd for k = 1:length(nlp.mask)
...
function NLPModels.hess_structure!(
  ...
  return nlp.hess_rows, nlp.hess_cols
end
